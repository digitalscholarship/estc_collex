			...
			page.rows.each(function (el){
				...
				el.each(function (subel) {
                   ...
                   addColorPick(row, subel.colorpick, subel.klass, subel.callback, subel.value);
						...
						if(subel.rows)
							textarea.writeAttribute({ rows: subel.rows});
						if(subel.cols)
							textarea.writeAttribute({ cols: subel.cols});
						...
						var file_input2 = new Element('input', { id: GeneralDialog.makeId(subel.file), type: 'file', name: subel.file, multiple: true});
							if (subel.size)
...
var RteInputDlg = Class.create({
	initialize: function (params) {
		...
		var radiobuttons = params.radiobuttons;
		...
		this.ok = function(event, params)
		{
			...
			console.log(data);
			if(radiobuttons!== undefined && radiobuttons == true){
				okCallback(data.gd_textareaValue, data.radioList, data.document);
			}
			else
		...
		if(radiobuttons!== undefined && radiobuttons == true){
			var btnRow = dlgLayout.rows[1];
			dlgLayout.rows[1] = [{ radioList: 'radioList', klass: 'agree_disagree', options: [ {text: 'Agree', value: '3'}, {text: 'Disagree', value: '4'}], value: '3'}];
			dlgLayout.rows[2] = [ { file: 'document', size: 40 } ];
			dlgLayout.rows[3] = btnRow; 
		}
...
var addFeedbackDlg = null;
function stopReviewInfo(errMessage){
	if (errMessage.startsWith('OK:'))
		addFeedbackDlg.fileUploadFinished(errMessage.substring(3));
	else
		addFeedbackDlg.fileUploadError(errMessage);
	return true;
}

var feedbackDlg = Class.create({
	initialize: function (url, annotationid, arrval) {
		this.class_type = 'FeedbackDlg';	// for debugging
		var This = this;
		var dlg = null;
		...
		{
			addFeedbackDlg = This;
			//dlg.setFlash('Adding review info...', false);
		...
		this.fileUploadFinished = function(message) {
			dlg.setFlash(message, false);
			reloadPage();
		};
		var show = function () {
			var dlgLayout = { 
			page: 'layout',
			rows: [
				[ { textarea: 'feedback', cols: '69', rows: '10' }],
				[ { radioList: 'opinion', klass: 'agree_disagree', options: arrval, value: arrval[0].value}],
				[ { file: 'documents[]', size: 40 }],
				[ { hidden: 'annotationid', value: annotationid, type: "hidden" }],
				[ { rowClass: 'gd_last_row' }, { button: 'Ok', callback: sendWithAjax, isDefault: true }, { button: 'Cancel', callback: GeneralDialog.cancelCallback } ]					
			]
		}; 
			var params = { this_id: "gd_text_input_dlg", pages: [ dlgLayout ], body_style: "gd_message_box_dlg", row_style: "gd_message_box_row", title: "Provide your feedback" };
